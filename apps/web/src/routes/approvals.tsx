...
const lineItemsMap: Record<string, { sku: string; name: string; qty: number; unit: string; unitPrice: number }[]> = {
  a1: [
    { sku: "WUR-0042-45", name: "Spanplattenschraube Torx 4.5×40", qty: 500, unit: "pc",  unitPrice: 0.06 },
    { sku: "WUR-0091-30", name: "Dübel Nylon UX 8×50",             qty: 200, unit: "pc",  unitPrice: 0.18 },
    { sku: "HIL-TAPE-19", name: "Gewebeband silber 19mm × 50m",    qty: 10,  unit: "rl",  unitPrice: 5.40 },
    { sku: "WUR-BAT-AA",  name: "Batterie Alkaline AA",            qty: 48,  unit: "pc",  unitPrice: 0.65 },
  ],
  a3: [
    { sku: "HIL-TAPE-19", name: "Gewebeband silber 19mm × 50m",    qty: 20,  unit: "rl",  unitPrice: 5.40 },
    { sku: "DEB-DR-6",    name: "Bohrer SDS-Plus 6×160",           qty: 80,  unit: "pc",  unitPrice: 4.20 },
    { sku: "WUR-0091-30", name: "Dübel Nylon UX 8×50",             qty: 200, unit: "pc",  unitPrice: 0.18 },
  ],
  a4: [
    { sku: "DEB-DR-6",    name: "Bohrer SDS-Plus 6×160",           qty: 50,  unit: "pc",  unitPrice: 4.20 },
    { sku: "WUR-0042-45", name: "Spanplattenschraube Torx 4.5×40", qty: 800, unit: "pc",  unitPrice: 0.06 },
    { sku: "WUR-0091-30", name: "Dübel Nylon UX 8×50",             qty: 300, unit: "pc",  unitPrice: 0.18 },
    { sku: "WUR-BAT-AA",  name: "Batterie Alkaline AA",            qty: 48,  unit: "pc",  unitPrice: 0.65 },
  ],
  a6: [
    { sku: "PUA-FOAM-1",  name: "PU-Schaum 750ml Standard",        qty: 60,  unit: "can", unitPrice: 6.80 },
    { sku: "PUA-SIL-310", name: "Silikon sanitär weiss 310ml",     qty: 80,  unit: "tb",  unitPrice: 4.10 },
    { sku: "HG-PPE-G09",  name: "Arbeitshandschuhe Nitril Gr. 9",  qty: 48,  unit: "pr",  unitPrice: 1.90 },
    { sku: "HG-PPE-M01",  name: "FFP2 Atemschutzmaske",            qty: 80,  unit: "pc",  unitPrice: 0.85 },
  ],
  a8: [
    { sku: "HIL-TAPE-19", name: "Gewebeband silber 19mm × 50m",    qty: 12,  unit: "rl",  unitPrice: 5.40 },
    { sku: "DEB-DR-6",    name: "Bohrer SDS-Plus 6×160",           qty: 40,  unit: "pc",  unitPrice: 4.20 },
  ],
};
...
const foremanContext: Record<string, { role: string; todaySpend: number; ordersToday: number; dailyLimit: number }> = {
  "M. Keller":  { role: "Site foreman – Letzigrund Tower B", todaySpend: 312.40, ordersToday: 2, dailyLimit: 400 },
  "A. Brunner": { role: "Site foreman – Sihlcity Refit",     todaySpend: 452.00, ordersToday: 3, dailyLimit: 250 },
  "L. Studer":  { role: "Site foreman – Hardbrücke Depot",   todaySpend: 583.30, ordersToday: 4, dailyLimit: 300 },
  "R. Frei":    { role: "Site foreman – Oerlikon School",    todaySpend: 921.50, ordersToday: 2, dailyLimit: 200 },
};
            ...
            <tbody>
              {rows.map((a) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-muted-foreground text-xs whitespace-nowrap">{a.submittedAt}</td>
                  ...
                  <td className="px-5 py-3 whitespace-nowrap">
                    <span className={["text-mono text-[10px] uppercase tracking-wider px-2 py-1 rounded", thresholdColors[a.threshold]].join(" ")}>
                      {thresholdLabel[a.threshold]}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-right tabular whitespace-nowrap">{a.items}</td>
                  <td className="px-5 py-3 text-right tabular font-medium whitespace-nowrap">{CHF(a.total)}</td>
          ...
          <div className="w-[500px] max-w-full bg-background border-l border-border flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-secondary/30 shrink-0">
              ...
              <div className="flex items-center gap-2">
                <span className={["text-mono text-[10px] uppercase tracking-wider px-2 py-1 rounded", thresholdColors[selected.threshold]].join(" ")}>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              {/* Approval reason */}
              {(selected.threshold === "pm" || selected.threshold === "central") && selected.status === "pending" && (
                <div className={[
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <div className={["text-sm font-semibold", selected.threshold === "central" ? "text-destructive" : "text-warning-foreground"].join(" ")}>
              ...
              <div className="rounded-md border border-border p-4">
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 text-xs border-t border-border pt-3">
                  <div>
                    ...
                    <div className="font-semibold tabular">{CHF(foremanContext[selected.foreman]?.todaySpend ?? selected.total)}</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground mb-0.5">Daily limit</div>
                    <div className="font-semibold tabular">{CHF(foremanContext[selected.foreman]?.dailyLimit ?? 250)}</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground mb-0.5">Orders today</div>
                    <div className="font-semibold">{foremanContext[selected.foreman]?.ordersToday ?? 1}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-md border border-border p-4">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">Framework contract · C-materials procurement</div>
              </div>
                    ...
                    <tbody>
                      {(lineItemsMap[selected.id] ?? []).map((li, i) => (
                        <tr key={i} className="border-t border-border">
                          ...
                          <td className="px-3 py-2 text-right tabular">{li.qty} {li.unit}</td>
                          <td className="px-3 py-2 text-right tabular">{CHF(li.unitPrice)}</td>
                          <td className="px-3 py-2 text-right tabular font-medium">{CHF(li.qty * li.unitPrice)}</td>
                        </tr>
                      ))}
                      ...
                      <tr className="border-t-2 border-border bg-secondary/50 font-semibold text-sm">
                        <td colSpan={4} className="px-3 py-2.5 text-right">Order total</td>
