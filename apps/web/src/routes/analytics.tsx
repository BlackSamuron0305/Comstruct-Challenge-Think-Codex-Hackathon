...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Tail-spend analytics: by project, supplier, product group and foreman.",
      },
    ],
  }),
  component: Analytics,
...
function Analytics() {
  ...
  const projectRows = [...projectSpend.entries()]
    .map(([project, spend]) => ({ project, spend }))
    .sort((a, b) => b.spend - a.spend);
  const supplierRows = [...supplierSpend.entries()]
    .map(([name, spend]) => ({ name, spend }))
    .sort((a, b) => b.spend - a.spend)
    .slice(0, 6);
  const groupRows = [...groupSpend.entries()]
    .map(([group, value]) => ({ group, value }))
    .sort((a, b) => b.value - a.value)
    .slice(0, 8);
  const trendRows = recentDays.map((day) => ({
    week: dayLabel(day),
    spend: spendTrend.get(day) ?? 0,
  }));
  ...
  return (
    <DashboardLayout
      title="Analytics"
      subtitle="Live tail-spend visibility across projects, suppliers and groups"
    >
      {isLoading ? (
